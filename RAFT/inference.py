...
def inference_images(args, image1, image2):
    # get the RAFT model
    model = RAFT(args)
    # load pretrained weights
    pretrained_weights = torch.load(args.model)
    ...
    frame_1 = cv2.imread(image1)
    frame_2 = cv2.imread(image2)
    ...
    flow_low, flow_up = model(frame_1, frame_2, iters=args.iters, test_mode=True)
    # transpose the flow output and convert it into numpy array
    ret = vizualize_flow(frame_1, flow_up, True, 0)
...
def main():
    ...
    inference_images(args, "demo-frames/frame_0016.png", "demo-frames/frame_0017.png")
